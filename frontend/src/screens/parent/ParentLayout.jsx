    ...
    <div className="min-h-screen bg-slate-100 flex flex-col">
      ...
      <div className="bg-blue-700 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-20 shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-200 text-xs">{t('parent.panelTitle')}</p>
          </div>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-20">
        <div className="max-w-2xl mx-auto flex justify-around">
          {[
            { to: '/parent',          icon: Home,    label: t('nav.dashboard'), end: true },
            { to: '/parent?tab=stats', icon: BarChart3, label: t('nav.stats') },
            { to: '/parent/children/new', icon: Gift, label: t('nav.child') },
            { to: '/parent/contracts/new', icon: Settings, label: t('nav.contract') },
